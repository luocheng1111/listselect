package com.demo.library.listsellibrary.view;
...
//            "K", "L", "M", "N", "O", "P", "Q", "R", "S", "T", "U", "V", "W", "X", "Y", "Z", "#"};
    ...
    public void setDataSource(List<String> tags){
        indexArray = listRemoveDuplicate(tags);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.AT_MOST || widthMode == MeasureSpec.AT_MOST) {
            ...
            if (heightMode == MeasureSpec.AT_MOST) {
                heightSize = (maxHeight + 20) * indexArray.size();
            }
    ...
    protected void onDraw(Canvas canvas) {
        for (int i = 0; i < indexArray.size(); i++) {
            String index = indexArray.get(i);
            float x = (mWidth - getTextWidth(mTextPaint, index)) / 2;
            float y = mMarginTop + mHeight * i + (mHeight + getTextHeight(mTextPaint, index)) / 2;
...
//                    setBackgroundColor(TOUCH_COLOR);
                    setBackgroundResource(R.drawable.sellist_sidebar_touchbg);
                    if (onSideBarTouchListener != null) {
                        for (int i = 0; i < tags.size(); i++) {
                            if (indexArray.get(pos).equals(tags.get(i))) {
                                onSideBarTouchListener.onTouch(indexArray.get(pos), i);
                                break;
                            }
                            if (i == tags.size() - 1) {
                                onSideBarTouchListener.onTouch(indexArray.get(pos), -1);
                            }
                        }
                    }
                }
    ...
    private void getMaxTextSize() {
        for (String index : indexArray) {
            ...
            maxHeight = Math.max(maxHeight, getTextHeight(mTextPaint, index));
        }
    }
    ...
    //list去重
    public static List<String>  listRemoveDuplicate(List<String> list) {
